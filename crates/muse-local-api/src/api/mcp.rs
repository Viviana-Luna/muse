//! MCP Server 管理接口，统一读写受保护的 `config.toml` 事实源。
...
pub(super) fn routes() -> ApiRouter {
    Router::new()
        .route(
            "/mcp/servers",
            get(handle_mcp_servers).post(handle_create_mcp_server),
        )
        .route("/mcp/servers/test-draft", post(handle_test_mcp_draft))
        .route(
            "/mcp/servers/{name}",
        ...
        .route("/mcp/servers/{name}/test", post(handle_test_mcp_server))
        .route(
            "/mcp/servers/{name}/refresh",
            post(handle_refresh_mcp_server),
        )
        .route("/mcp/servers/{name}/tools", get(handle_mcp_server_tools))
        .route(
            "/mcp/servers/{name}/resources",
            get(handle_mcp_server_resources),
        )
}
...
fn mcp_config_error(
    ...
    match error {
        ...
        MuseConfigStoreError::UnsupportedVersion(_) => {
            bad_request("config_version_unsupported：config.toml 版本高于当前 Muse 支持范围。")
        }
        MuseConfigStoreError::Io(_)
        | MuseConfigStoreError::Parse(_)
        | MuseConfigStoreError::Serialize(_) => internal_error(
            "config_write_failed：读写 config.toml 失败，请检查语法、权限和磁盘状态。".to_string(),
        ),
    }
}
...
fn secret_state(target: String, configured: bool) -> McpSecretFieldState {
    McpSecretFieldState { target, configured }
}
    ...
    let transport = match profile.transport.as_str() {
        ...
        _ => {
            ...
            let header_secrets = profile
                .secret_headers
                .keys()
                .filter(|target| target.as_str() != "Authorization" || bearer_token.is_none())
                .cloned()
                .map(|target| secret_state(target, true))
                .collect();
...
pub(crate) async fn handle_mcp_servers(
    ...
    for (name, profile) in profiles.mcp_servers {
        ...
        output.push(McpServerSummaryResponse {
            ...
            resource_count: check
                .as_ref()
                .map(|value| value.resource_count)
                .unwrap_or(0),
            ...
            Some(source_name) => {
                ...
                if mcp_server_revision(profile) != expected {
                    return Err((
                        ...
                        Json(ErrorResponse {
                            error:
                                "mcp_revision_conflict：MCP 连接已在其他窗口中变更，请刷新后重试。"
                                    .to_string(),
                        }),
                    ));
                }
...
mod mcp_management_tests {
    ...
    fn keep_replace_and_delete_use_target_as_the_secret_key() {
        ...
        let (_, deleted) = build_mcp_profile(delete_request, Some(&existing)).expect("应删除旧值");
    ...
    fn bearer_token_is_saved_as_secret_authorization_header() {
        ...
        assert_eq!(profile.secret_headers["Authorization"], "Bearer api-key");
    }
