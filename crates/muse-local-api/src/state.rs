...
fn build_runtime_mode_context(mode_state: RuntimeModeState) -> String {
    match mode_state.tool_preset() {
        muse_core::domain::runtime::ToolPreset::Daily => {
            "【运行模式】\n当前模式：日常模式。\n目标：优先保持角色陪伴、自然对话、语音互动和低打扰轻工具。可以读取、查找、轻量编辑文件或在用户明确需要时联网；不要主动推进多步骤实现、命令执行或高风险操作。若任务明显需要多文件修改、命令验证、MCP 综合检索或长期执行，应先说明需要进入专注模式并等待用户确认。".to_string()
        }
        muse_core::domain::runtime::ToolPreset::FocusPlan => {
            "【运行模式】\n当前模式：专注模式。\n当前工具预设：计划。\n目标：围绕用户目标读文件、查资料、梳理任务和提出计划。计划预设只能使用读查问和只读 MCP 资源，不直接改代码、不执行命令、不触发写入动作。是否进入计划预设应由用户明确要求、任务风险或模型判断触发；计划不是专注模式的必经阶段。".to_string()
        }
        muse_core::domain::runtime::ToolPreset::FocusBuild => {
            "【运行模式】\n当前模式：专注模式。\n当前工具预设：专注工作。\n目标：以任务纪律优先处理复杂工作，可在审批边界内执行编辑、命令、MCP 工具和验证。需要先规划时再进入计划预设；遇到审批、风险或计划外变更时先停下说明。".to_string()
        }
    }
}
