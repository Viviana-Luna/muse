...
"#,
        ...
        std::fs::write(config_dir.join("config.toml"), &manually_edited)
            .expect("应模拟手工编辑配置");
    ...
    async fn persona_update_removes_only_unreferenced_uploaded_asset() {
        ...
        std::fs::write(uploaded_dir.join(&old_avatar_name), b"old-avatar")
            .expect("应写入待清理头像");
    ...
    async fn chat_stream_emits_structured_tool_status_sequence() {
        ...
        let payloads = String::from_utf8(body.to_vec())
            ...
            .map(|line| {
                serde_json::from_str::<serde_json::Value>(line).expect("SSE data 应为 JSON")
            })
            .collect::<Vec<_>>();
        ...
        assert!(status_payloads.iter().all(|payload| {
