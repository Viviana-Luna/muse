...
fn test_execution_policy() -> muse_runtime::FrozenExecutionPolicy {
    let root = std::env::current_dir()
        ...
        .canonicalize()
        .expect("应能解析测试工作区");
    muse_runtime::FrozenExecutionPolicy::new("request_approval", "workspace_write", vec![root])
}
...
fn parses_emotion_prefix_across_chunks() {
    ...
    match state.push_chunk("}\n你好") {
        PrefixParseResult::Resolved { emotion, text } => {
            assert_eq!(
                emotion.as_ref().map(|value| value.emotion.as_str()),
                Some("happy")
            );
            assert_eq!(text, "你好");
        }
...
fn stream_prefix_waits_for_reasoning_block_to_close() {
    ...
    match state.push_chunk(
        "推理</think> {\"emotion\":\"neutral\",\"intensity\":0,\"reason_code\":\"neutral\"}你好",
    ) {
        PrefixParseResult::Resolved { emotion, text } => {
            assert_eq!(
                emotion.as_ref().map(|value| value.emotion.as_str()),
...
fn exa_web_search_stays_visible_and_normalizes_free_and_api_failures() {
    ...
    muse_core::domain::tool::builtin::register_all(
        &mut Arc::get_mut(&mut state).expect("测试状态尚未共享").tools,
    );
    let mcp_catalog = muse_core::domain::mcp::McpToolCatalog::empty_for_config_path(
        data_dir.join("mcp").join("servers.json"),
    );

    let defs = super::runtime_frozen_tool_defs_for_policy_with_catalog(&state, None, &mcp_catalog);
    assert!(
        defs.iter()
...
"#;
    ...
    for endpoint in [
        ...
        assert!(
            super::build_trusted_search_client(endpoint, std::time::Duration::from_secs(1),)
                .is_ok(),
...
fn runtime_tool_capability_matrix_matches_handler_registry() {
    for capability in crate::runtime_support::tool_adapters::capabilities() {
        ...
        let handler = super::runtime_tool_handler(capability.name)
            .unwrap_or_else(|| panic!("能力矩阵中的 `{}` 必须存在真实 handler", capability.name));
...
fn runtime_skill_catalog_marks_effective_source_and_filters_policy() {
    ...
    let (builtin_catalog, omitted) =
        super::effective_runtime_skill_catalog(&data_dir, &preferences, &Default::default());
    assert_eq!(omitted, 0);
    ...
    let (disabled_catalog, _) =
        super::effective_runtime_skill_catalog(&data_dir, &preferences, &disabled_policy);
...
async fn skill_tool_loads_workspace_skill_and_rejects_path_like_names() {
    ...
    let invalid_result = super::tool_skill_from_workspace(&turn, &invalid, &workspace).await;
...
async fn user_skill_load_uses_frozen_catalog_revision_and_records_audit_fields() {
    ...
    turn.runtime_policy.skill_catalog = vec![muse_core::domain::turn::RuntimeSkillCatalogEntry {
...
async fn builtin_skill_loads_through_frozen_catalog() {
    ...
    let preferences = config.skill_preferences().clone();
    let builtin =
        muse_core::domain::skill::builtin_skill("skill-creator").expect("应注册内置 skill-creator");
...
async fn create_skill_uses_store_and_becomes_loadable_next_turn() {
    ...
    let current_load = super::tool_skill(
        &state,
        &current_turn,
        &test_tool_call(
            "load_skill",
    ...
    blocked_turn.skill_policy.mode = muse_core::domain::persona::ResourcePolicyMode::Disabled;
...
async fn selected_skill_is_loaded_from_frozen_catalog_before_model_call() {
    ...
    let required_tools =
        super::selected_builtin_skill_required_tools(&skill_catalog, Some("skill-creator"));
    assert_eq!(required_tools, ["create_skill"]);
    let mut full_tool_defs =
        super::runtime_frozen_tool_defs_for_policy_with_catalog(&state, None, &mcp_catalog);
    assert!(
        full_tool_defs
            .iter()
    ...
    let handler =
        super::runtime_tool_handler("create_skill").expect("运行时应注册 create_skill handler");
    assert!(
        handler
            .check_permissions(&state, &turn, &create_call)
            .is_ok(),
        "显式 Skill 工具授权必须穿过实际 handler 权限门禁"
    );

    let (prompt, activated) = super::activate_selected_skill(&state, &turn, "skill-creator")
        .await
...
async fn command_run_rejects_known_process_tree_escape_before_spawn() {
    ...
    let result =
        super::tool_command_run(&test_execution_policy(), &call, None, false, &cancel_token).await;
...
fn file_search_never_bypasses_frozen_roots() {
    ...
    assert_eq!(
        super::resolve_file_search_base(&policy, ".").expect_err("默认工作区不在冻结根时必须拒绝"),
...
async fn command_audit_streams_to_private_file_and_caps_each_stream_at_16_mib() {
    ...
    let prepared =
        super::prepare_command_audit_files_in(&temp).expect("应能在受控测试数据目录创建审计文件");
...
async fn hard_deadline_drop_kills_the_complete_command_process_group() {
    ...
    let result =
        tokio::time::timeout(std::time::Duration::from_millis(100), command.as_mut()).await;
...
fn dispatch_rechecks_permission_revocation_without_accepting_mid_turn_expansion() {
    ...
    let frozen_restricted =
        muse_runtime::TurnSnapshot::new(test_turn_context("turn-policy-no-expansion"), definitions)
            .with_execution_policy(revoked.clone());
    let expanded =
        muse_runtime::FrozenExecutionPolicy::new("full_access", "danger_full_access", vec![root]);
    let result = super::execution_boundary_for_dispatch(
        &frozen_restricted,
        &expanded,
        &call,
...
fn full_access_does_not_bypass_frozen_mcp_write_approval() {
    let policy =
        muse_runtime::FrozenExecutionPolicy::new("full_access", "danger_full_access", Vec::new());
...
fn legacy_daily_mode_request_migrates_to_default_work_state() {
    let state = super::parse_runtime_mode_state("daily", None).expect("旧 daily 请求应平滑迁移");
...
async fn canonical_tool_events_survive_store_reopen_with_equivalent_provider_frames() {
    ...
    let canonical_command_arguments = super::canonical_session_tool_arguments(&command_arguments);
...
fn runtime_tool_context_effect_can_replace_model_result() {
    ...
    let compact_context = super::tool_result_content_for_model("session_compact", &compact_result);
...
fn skill_catalog_budget_is_bounded() {
    ...
    let (catalog, omitted) = super::freeze_skill_catalog(summaries, &Default::default());
...
fn restricted_https_client_structurally_disables_system_proxy() {
    ...
    assert!(super::build_pinned_https_client(&target, std::time::Duration::from_secs(1)).is_ok());
}
...
fn supported_chat_provider_clamps_provider_limit() {
    ...
    assert_eq!(
        super::normalize_chat_max_tokens("volcengine_agent_plan", 0),
        1
    );
    assert_eq!(
...
async fn provider_chat_probe_maps_auth_failure_without_body() {
    ...
    let error =
        super::probe_provider_chat(&format!("http://{address}"), "test-model", "invalid-key")
            .await
...
async fn model_catalog_crud_rejects_deleting_the_active_model() {
    ...
    let state = build_test_state(&config_dir);
    let created =
        super::handle_create_catalog_model(State(state.clone()), Json(glm_catalog_draft()))
            .await
            .expect("应能创建模型")
            .0;
...
async fn provider_key_is_written_to_toml_and_never_returned_by_model_apis() {
    ...
    let content =
        std::fs::read_to_string(config_dir.join("config.toml")).expect("应读取 Provider Profile");
...
fn visual_pack_patch_updates_one_image_slot_without_overwriting_others() {
    ...
    assert_eq!(
        updated.background_path,
        "/api/assets/uploaded/background.webp"
    );
...
fn persona_mcp_allow_list_is_frozen_before_global_tool_policy() {
    ...
    for server in ["alpha", "beta"] {
        catalog
            .tools
            .push(muse_core::domain::mcp::ExternalMcpToolDef {
    ...
    let allowed =
        super::runtime_frozen_tool_defs_for_policy_with_catalog(&state, Some(&persona), &catalog);
    assert_eq!(
        allowed
            .iter()
    ...
    let skill_disabled =
        super::runtime_frozen_tool_defs_for_policy_with_catalog(&state, Some(&persona), &catalog);
    assert!(!skill_disabled.iter().any(|tool| matches!(
        tool.name.as_str(),
        "load_skill" | "create_skill" | "use_skill" | "skill"
    )));
...
async fn turn_freezes_mcp_approval_revision_and_annotations_hash() {
    ...
    configure_test_chat(&state).await;
    let mut catalog =
        muse_core::domain::mcp::McpToolCatalog::empty_for_config_path(dir.join("config.toml"));
    catalog
        .tools
        .push(muse_core::domain::mcp::ExternalMcpToolDef {
            ...
            approval_policy: muse_core::domain::mcp::McpApprovalPolicy::TrustedReadOnly,
            approval_source: muse_core::domain::mcp::McpApprovalSource::ServerPolicy,
    ...
    catalog.tools[0].approval_policy = muse_core::domain::mcp::McpApprovalPolicy::AlwaysAsk;
...
async fn runtime_policy_snapshot_contains_only_frozen_public_facts() {
    ...
    assert_eq!(
        turn.runtime_policy.persona_version.as_deref(),
        Some("2.0.0")
    );
    assert_eq!(
        turn.runtime_policy.skill_policy.allowed_skills,
        vec!["calendar"]
    );
...
async fn turn_skill_catalog_is_isolated_policy_filtered_and_frozen() {
    ...
    disabled_persona.skill_policy.mode = muse_core::domain::persona::ResourcePolicyMode::Disabled;
...
fn aggregated_sync_test_cases() {
    let mut failures = Vec::new();
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            persona_emotion_persistence_policy_is_frozen_at_commit,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_context_snapshot_splits_context_segments,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            deepseek_models_use_large_context_defaults,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            token_usage_day_range_starts_at_local_midnight,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            parses_emotion_prefix_across_chunks,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            returns_plain_text_when_no_prefix_exists,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            strips_reasoning_block_before_emotion_prefix,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            stream_prefix_waits_for_reasoning_block_to_close,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            tool_prefix_accepts_direct_name_arguments_json,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            tool_prefix_keeps_plain_json_as_text_when_it_is_not_a_tool_call,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            compact_prompt_requires_recoverable_summary_sections,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            rule_compact_summary_preserves_tool_results_and_next_step,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            exa_web_search_stays_visible_and_normalizes_free_and_api_failures,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_run_schema_matches_runtime_timeout_and_audit_contract,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_tool_capability_matrix_matches_handler_registry,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            builtin_skill_keeps_priority_and_user_shadowing,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_skill_catalog_marks_effective_source_and_filters_policy,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            selected_skill_request_name_is_normalized_and_validated,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_tool_handlers_own_approval_summaries,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_run_handler_summarizes_dangerous_commands,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_risk_classifier_keeps_safe_commands_quiet,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            file_search_never_bypasses_frozen_roots,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            file_search_rejects_symlink_escape,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_output_summary_preserves_head_and_tail_with_strict_byte_limit,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_audit_rejects_symlinked_control_directory,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_audit_resource_reads_by_logical_uri_without_exposing_path,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_audit_resource_rejects_traversal,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_audit_resource_read_does_not_create_missing_directory,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_audit_directory_permissions_are_private,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_audit_resource_rejects_symlink_file,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_tool_handlers_check_workspace_boundary_before_dispatch,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            dispatch_rechecks_permission_revocation_without_accepting_mid_turn_expansion,
        ))
        .is_err()
        {
            failures.push(
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_tool_handlers_declare_readonly_and_mutating_policy,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            active_turn_mode_transition_uses_the_completed_tool_result,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_tool_handlers_validate_inputs_before_dispatch,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            mcp_resource_registry_lists_local_harness_resources,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            legacy_mcp_names_are_read_aliases_but_new_catalog_uses_muse,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_transcript_replay_restores_model_context,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_transcript_replay_only_publishes_committed_turns,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_transcript_replay_drops_an_unfinished_v3_working_copy,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_transcript_replay_recovers_a_hard_crash_after_effect_dispatch_started,
        ))
        .is_err()
        {
            failures.push(
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            replay_compatibly_treats_legacy_argument_audit_as_unknown_arguments,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_transcript_replay_filters_conversation_and_supports_fork_cutoff,
        ))
        .is_err()
        {
            failures
                .push("runtime_transcript_replay_filters_conversation_and_supports_fork_cutoff");
        }
    }
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_transcript_replay_restores_latest_task_state,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_transcript_replay_restores_latest_todo_state,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_transcript_delete_filters_only_target_conversation,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_session_list_for_active_hides_empty_legacy_default_placeholder,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_conversation_file_component_rejects_path_separators,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_tool_handlers_render_model_results,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            externalized_tool_result_keeps_reference_instead_of_full_body,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_tool_context_effect_appends_state_changes_for_model,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_tool_context_effect_can_replace_model_result,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            skill_result_enters_model_context_once,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            skill_catalog_budget_is_bounded,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            chat_status_payload_uses_structured_summary_fields,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            approval_resolved_event_preserves_reason,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            command_output_delta_event_uses_stable_payload_shape,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            runtime_prompt_uses_persona_and_tools,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            uploaded_image_extension_accepts_supported_mime_types,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            uploaded_asset_name_accepts_only_content_hash_and_supported_extension,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            speech_upload_uses_magic_bytes_and_rejects_type_mismatch,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            speech_upload_rejects_oversized_payload_before_decoding,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            web_fetch_rejects_private_and_loopback_addresses,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            restricted_https_client_structurally_disables_system_proxy,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            full_access_does_not_bypass_frozen_mcp_write_approval,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            auto_mode_routes_required_tools_to_reviewer_instead_of_hardcoded_bypass,
        ))
        .is_err()
        {
            failures
                .push("auto_mode_routes_required_tools_to_reviewer_instead_of_hardcoded_bypass");
        }
    }
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            canonical_tool_session_preserves_semantics_but_redacts_detected_secrets,
        ))
        .is_err()
        {
            failures
                .push("canonical_tool_session_preserves_semantics_but_redacts_detected_secrets");
        }
    }
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            supported_chat_provider_clamps_provider_limit,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            content_hash_hex_is_stable_for_same_bytes,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            visual_pack_patch_updates_one_image_slot_without_overwriting_others,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            visual_pack_patch_persists_theme_without_images,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            persona_mcp_allow_list_is_frozen_before_global_tool_policy,
        ))
        .is_err()
        {
    ...
    {
        if std::panic::catch_unwind(std::panic::AssertUnwindSafe(
            resolves_visual_pack_for_persona,
        ))
        .is_err()
        {
...
async fn aggregated_async_test_cases() {
    ...
    let mut failures = Vec::new();
    {
        if std::panic::AssertUnwindSafe(durable_commit_failure_blocks_success_publication())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(intermediate_tool_event_failure_is_propagated())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    }
    {
        if std::panic::AssertUnwindSafe(
            external_effect_fact_precedes_tool_result_persistence_failure(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            effect_boundary_persistence_failure_blocks_dispatch_marking(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(effect_fact_mark_failure_blocks_real_dispatch())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(hard_deadline_drop_cleans_pending_approval_registration())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(
            event_send_failure_cleans_pending_user_question_registration(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            active_turn_guard_cleans_all_pending_interactions_for_its_turn(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            skill_tool_loads_workspace_skill_and_rejects_path_like_names(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            user_skill_load_uses_frozen_catalog_revision_and_records_audit_fields(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    #[cfg(unix)]
    {
        if std::panic::AssertUnwindSafe(skill_tool_rejects_symlinked_compatibility_paths())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(command_output_reader_never_blocks_on_a_full_sse_channel())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(
            command_audit_streams_to_private_file_and_caps_each_stream_at_16_mib(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            command_audit_sink_uses_bounded_queue_and_counts_dropped_bytes(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(command_run_cancels_running_child_process())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(
            hard_deadline_drop_kills_the_complete_command_process_group(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            command_run_cleans_background_process_after_root_shell_exits(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    #[cfg(unix)]
    {
        if std::panic::AssertUnwindSafe(sse_disconnect_cancels_turn_and_running_command())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(runtime_tool_agent_registers_lightweight_subtask())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(
            runtime_tool_task_stop_records_current_turn_cancellation_result(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            canonical_tool_events_survive_store_reopen_with_equivalent_provider_frames(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    #[cfg(feature = "live-tests")]
    {
        if std::panic::AssertUnwindSafe(provider_model_fetch_preserves_upstream_auth_failure())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    #[cfg(feature = "live-tests")]
    {
        if std::panic::AssertUnwindSafe(provider_chat_probe_accepts_valid_assistant_message())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    #[cfg(feature = "live-tests")]
    {
        if std::panic::AssertUnwindSafe(provider_chat_probe_maps_auth_failure_without_body())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(
            agent_plan_model_fetch_without_key_fails_before_upstream_request(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(model_catalog_crud_rejects_deleting_the_active_model())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(
            active_model_switch_requires_the_target_provider_credential_locally(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            provider_key_is_written_to_toml_and_never_returned_by_model_apis(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            appearance_refresh_cannot_consume_manual_model_revision_without_rebuild(),
        )
        .catch_unwind()
        .await
        .is_err()
        {
            failures
                .push("appearance_refresh_cannot_consume_manual_model_revision_without_rebuild");
        }
    }
    {
        if std::panic::AssertUnwindSafe(
            current_model_summary_preserves_provider_and_model_display_names(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            catalog_delete_waits_for_the_shared_model_configuration_gate(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(
            create_persona_does_not_persist_visual_pack_when_persona_is_invalid(),
        )
        .catch_unwind()
        .await
        .is_err()
    ...
    {
        if std::panic::AssertUnwindSafe(runtime_policy_snapshot_contains_only_frozen_public_facts())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(turn_resolves_persona_model_and_records_explicit_fallback())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(turn_skill_catalog_is_isolated_policy_filtered_and_frozen())
            .catch_unwind()
            .await
            .is_err()
        {
    ...
    {
        if std::panic::AssertUnwindSafe(turn_freezes_mcp_approval_revision_and_annotations_hash())
            .catch_unwind()
            .await
            .is_err()
        {
