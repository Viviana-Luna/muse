//! 应用偏好接口，包含路由注册和 HTTP 适配实现。

use std::sync::Arc;

use axum::{Json, Router, extract::State, http::StatusCode, routing::get};
use muse_core::model::config::ModelsConfig;
...
use crate::state::{
    AppState, build_chat_provider, rebuild_speech_provider_from_state,
    rebuild_tts_provider_from_state,
};

pub(super) fn routes() -> ApiRouter {
    Router::new().route(
        "/preferences/appearance",
        get(handle_get_appearance_preferences).put(handle_put_appearance_preferences),
    )
}
...
async fn handle_put_appearance_preferences(
    State(state): State<Arc<AppState>>,
    ...
    if matches!(
        &result,
        Err(muse_core::app::preferences::MuseConfigStoreError::Conflict(
            _
        ))
    ) {
        rebuild_models_after_preferences_refresh(&state, &models).await?;
...
fn app_preferences_runtime_error(message: &str) -> (StatusCode, Json<ConfigMutationErrorResponse>) {
    (
        StatusCode::BAD_REQUEST,
        Json(ConfigMutationErrorResponse {
